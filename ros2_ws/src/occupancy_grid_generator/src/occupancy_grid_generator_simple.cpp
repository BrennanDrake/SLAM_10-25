/**
 * @file occupancy_grid_generator_simple.cpp
 * @brief Simplified implementation of occupancy grid generator without TF2
...
OccupancyGridGeneratorSimple::OccupancyGridGeneratorSimple(const rclcpp::NodeOptions & options)
: Node("occupancy_grid_generator_simple", options),
  ...
  robot_theta_(0.0)
{
  ...
  scan_sub_ = this->create_subscription<sensor_msgs::msg::LaserScan>(
    "/scan", 10, std::bind(&OccupancyGridGeneratorSimple::scan_callback, this, std::placeholders::_1));
  ...
  RCLCPP_INFO(this->get_logger(), "OccupancyGridGeneratorSimple initialized");
  RCLCPP_INFO(this->get_logger(), "Grid size: %dx%d, Resolution: %.3fm", 
              grid_width_, grid_height_, resolution_);
  RCLCPP_INFO(this->get_logger(), "Note: This simplified version assumes robot at origin (0,0,0)");
}

void OccupancyGridGeneratorSimple::initialize_grid()
{
...
void OccupancyGridGeneratorSimple::scan_callback(const sensor_msgs::msg::LaserScan::SharedPtr msg)
{
...
void OccupancyGridGeneratorSimple::update_grid(const sensor_msgs::msg::LaserScan::SharedPtr scan)
{
  // For simplified version, assume robot is at origin (0,0,0)
  // In a real implementation, you would get this from TF2
  
  // Convert robot position to grid coordinates
  int robot_grid_x, robot_grid_y;
  if (!world_to_grid(robot_x_, robot_y_, robot_grid_x, robot_grid_y)) {
  ...
  for (size_t i = 0; i < scan->ranges.size(); ++i) {
    ...
    // Calculate ray angle (assuming robot at 0 orientation)
    double ray_angle = robot_theta_ + scan->angle_min + i * scan->angle_increment;
    
    // Calculate end point of ray
    double end_x = robot_x_ + range * std::cos(ray_angle);
    double end_y = robot_y_ + range * std::sin(ray_angle);
    ...
    if (!world_to_grid(end_x, end_y, end_grid_x, end_grid_y)) {
      ...
      end_x = robot_x_ + max_range * std::cos(ray_angle);
      end_y = robot_y_ + max_range * std::sin(ray_angle);
      if (!world_to_grid(end_x, end_y, end_grid_x, end_grid_y)) {
...
bool OccupancyGridGeneratorSimple::world_to_grid(double x, double y, int & grid_x, int & grid_y) const
{
...
void OccupancyGridGeneratorSimple::bresenham_line(int x0, int y0, int x1, int y1, 
                                           std::vector<std::pair<int, int>> & free_cells)
{
  free_cells.clear();
...
RCLCPP_COMPONENTS_REGISTER_NODE(occupancy_grid_generator::OccupancyGridGeneratorSimple)
